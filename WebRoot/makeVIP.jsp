...
<title>卖花的小女孩--我要定制</title>
<%@ include file="head.txt"%>
<link rel="stylesheet" href="./css/head.css">
<link rel="stylesheet" type="text/css" href="css/style.css"
	media="screen" />
<link rel="stylesheet" href="./css/makevip.css">
<link
...
<script type="text/javascript">
						...
						function() {
							...
							if (n <= 1) {
								...
								$
										.ajax({
											type : "POST",
											url : 'customServlet?action=updateCustom',
	...
	function cost() {
		...
		if (chk_value.length == 0) {
			...
			$.ajax({
				type : "POST",
				url : 'customOrderServlet?action=addOrder',
				data : jsondata,
				success : function(msg) {
					alert("交易成功");
					location.href = "makeVIP.jsp";
				}
			});
		}
	}
...
<BODY style="background:#0A6A68;color:white">
	...
	<div class="container">
		<h1>
			我要定制 <IMG SRC="./icon/makevip.png" width="30px" height="30px">
		</h1>
		<!--表格的标题-->
		<div class="container">
			<%
				...
				if (id != null) {
					...
					boolean flag = CustomDao.judge(user.getId(), product.getF_id());
					int num = CustomDao.findNum(user.getId(), product.getF_id());
					if (flag) {
						CustomDao.updateOne(user.getId(), product.getF_id(), num);
					} else {
						CustomDao.insertOne(cart);
					}
				}

				ArrayList<Custom> cartss = CustomDao.findByUid(user.getId());
				double allMoney = 0;
								...
								<c:forEach items="${cartss}" var="cs">
									<tr>
										...
										<td><em>￥</em><span class="font_price" id="table_span"
											name="mww">${cs.flower.f_price}</span>
										</td>
										...
										<td><em>￥</em><label id="total">${cs.money}</label>
										</td>
										<td><a href="customServlet?action=deleteCustom&cid=${cs.cid}">删除</a>
										</td>
									</tr>
								</c:forEach>
								<tr>
									<td colspan="6"><a href="lookCosmetic.jsp">继续选择</a>
									</td>
								</tr>
							</tbody>
							<tfoot>
		...
		<div class="container">
			<input type='button' value='生成定制' onclick="javascript:cost()">
		</div>
	</div>
	...
	<div class="marque">
		...
		<div class="marqueone">
			<marquee direction="left" scrollamount="13">
				<%
					for(int i = 0 ; i < 6;i++){
						if (list.get(i).getF_id() != 0) {
				 %>
				<a href="showDetail.jsp?id=<%=list.get(i).getF_id()%>"><img src="<%=list.get(i).getF_picture()%>"></a>
				<%
					}}
				 %>
			</marquee>
		...
		<div class="marquetwo">
			<marquee direction="right" scrollamount="13">
				<%
					for(int i = 6 ; i < 12 ;i++){
						if (list.get(i).getF_id() != 0) {
				 %>
