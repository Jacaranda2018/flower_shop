...
public class CustomServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals("updateCustom")) {
			String cid = request.getParameter("cid");
			String num = request.getParameter("num");
			String money = request.getParameter("money");
			CustomDao.updateOnes(user.getId(), Integer.parseInt(cid),
					Integer.parseInt(num), Double.parseDouble(money));
		} else if (action.equals("deleteCustom")) {
			String id = request.getParameter("cid");
			int cid = Integer.parseInt(id);
			CustomDao.delById(cid);
			ArrayList<Custom> cartss = CustomDao.findByUid(user.getId());
			request.setAttribute("cartss", cartss);
			response.sendRedirect("makeVIP.jsp");
		}
	}
}
